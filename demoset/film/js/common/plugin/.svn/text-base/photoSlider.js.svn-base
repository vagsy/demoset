var photoSlider = (function(){	
	var imageSliderTem = 
				'<div class="img_slider_box">'+
					'<div id="imgSliderContBox" class="img_slider_cont_box">'+
					'<#for (var i=0;i<coll.length;i++) {#>'+
				    	'<div class="slider_cont" name="photo<#=i#>" index="<#=i#>">'+
							'<#var url=coll[i]#>'+
				        	'<img src="<#=url#>" data-src="<#=url#>" width="100%"/>'+
				        	//'<span id="test<#=i#>" style="color:#fff;margin-left:100px;">12312</span>'+
				    	'</div>'+
					'<#}#>'+
					'</div>'+
				'</div>'; 
	...
	var defaultOptions = {
			...
			cont_line_height:0,		//图片容器行间距	
			index : 0				//slider展现的第一个图片序号
	};
	var options = {};
	var wraper,sliderItemList,imageCount;
	function init(elId,collection,opts){
		...
		var render = template.compile(imageSliderTem);
		var html = render({"coll":collection});
		wraper = document.getElementById(elId);	//图片轮播容器元素
		wraper.innerHTML=html;
		sliderItemList = wraper.querySelectorAll(".slider_cont"); //图片集合
		imageCount = sliderItemList.length; //图片总数
		currIndex = parseInt(opts.index,10); //设置展现图片序号
		for(var i=0;i<imageCount;i++){
			var leftPx = i*-options.imageWidth; //图片距左边框的距离
			sliderItemList[i].style.cssText="left:"+leftPx+"px;width:"+options.imageWidth+"px;line-height:"+options.cont_line_height+"px";
			var showPrevIndex = currIndex==0?imageCount-1:currIndex-1;
			if(i == currIndex){
				addCssProperty(sliderItemList[i],transformProList,"translate(0px) translateZ(0px)");
				...
				var scale= eleIndex>currIndex?eleIndex-currIndex:eleIndex+imageCount-currIndex;//向右偏移倍数	
				addCssProperty(sliderItemList[i],transformProList,"translate("+options.imageWidth*scale+"px) translateZ(0px)");
			}
		}
