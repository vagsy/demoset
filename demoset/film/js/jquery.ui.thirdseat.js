...
;(function($){
	...
	function updataView(target){
		...
		$("li span",scroller).css({"width":seatWidth-4,"height":seatWidth-4});
	...
	function initView(target){
		...
		for(var i=0;i<row;i++){
			...
			vli.appendTo(vul);
			var seatrow=$("li:eq("+i+")",mapUl);
			for(var j=0;j<col;j++){
				var n=seatrow.children("span").eq(j);
				if(n.attr("row") && n.attr("row")!=""){
					vli.text(n.attr("row"));
					break;
				}
			}
		...
		var ls=$("li",mapUl);
		//ls.css("height",seatHeight);
		ls.each(function(){
			//var _this=this;
			$(this).css("height",seatHeight);
			$(this).children("span").css("width",seatWidth-4).css("height",seatWidth-4);			
		});
		refesh(target);
	...
	function isSequence(target){
		if($("input[name=sortMethod]",target).val()=="T"){
			return true;
		}else{
			return false;
		}
	...
	function getRow(target){
		return $("input[name=maxRow]",target).val();
	}
	function getCol(target){
		return $("input[name=maxCol]",target).val();
	}
	...
	function bindEvent(target){
		...
		$(target).unbind().bind('click',function(e){
			...
			if($(tt).hasClass("seatmap")){
				...
				seat.seat={"rowindex":rowIndex+1,"colindex":colIndex+1,"rowname":rowName,"colname":colName};
	...
	function selectSeat(target,param){
		...
		if(status>0){
			if(status==1){
				...
				alert("亲最多选择"+opts.maxSelected+"个座位哦");
			}
			return;
		}
